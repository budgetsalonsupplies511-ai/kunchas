...
test('pinned service categories appear first in the public booking catalog',async()=>{const {env,db}=fixture();db.exec("INSERT INTO services(id,name,category,sub_category,duration_minutes,price_cents,status) VALUES ('special-service','Special service','Special','General',30,2500,'Active'); INSERT INTO service_category_order(category,sort_order,pinned,updated_at) VALUES ('Hair',0,0,datetime('now')),('Special',1,1,datetime('now'))");const body=await (await publicBookingRoute(new Request('https://salon.test/api/public-booking/catalog'),env)).json();assert.equal(body.services[0].category,'Special');assert.equal(body.services[0].category_pinned,1);});

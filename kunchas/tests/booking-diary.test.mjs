...
const liveSource=readFileSync(new URL('../live-worker/index.js',import.meta.url),'utf8');
function client(bookings){
 ...
 vm.createContext(context);vm.runInContext(liveSource.slice(liveSource.indexOf('function diaryClock('),liveSource.indexOf('function openBookingDetail(')),context);
...
test('diary time labels sit inside rows and the current-time badge clears the time rail',()=>{
 const {context,table}=client([]);context.renderBookings();
 assert.equal((table.innerHTML.match(/<time class=/g)||[]).length,36);
 assert.match(table.innerHTML,/<time class="hour" style="top:9px">10:00 am<\/time>/);
 assert.match(table.innerHTML,/<time class="quarter" style="top:27px">10:15 am<\/time>/);
 assert.match(liveSource,/\.booking-now span \{[^}]*left:80px/);
});
test('short booking cards show start time and customer in one line',()=>{
 const short=booking('quick','17:15');short.duration_minutes=15;
 const {context,table}=client([short]);context.renderBookings();
 assert.match(table.innerHTML,/booking-card lane-0 online compact/);
 assert.match(table.innerHTML,/<strong>5:15 pm · Customer quick<\/strong>/);
 assert.match(table.innerHTML,/aria-label="Customer quick · Service · 5:15 pm/);
});
test('POS title does not repeat Kunchas when the branch name includes it',()=>{
 const context={};vm.createContext(context);
 vm.runInContext(liveSource.slice(liveSource.indexOf('function posBranchTitle('),liveSource.indexOf('function canCheckoutBooking(')),context);
 assert.equal(context.posBranchTitle({name:'Kunchas Ashfield'}),'Kunchas Ashfield');
 assert.equal(context.posBranchTitle({name:'Rockdale'}),'Kunchas Rockdale');
});

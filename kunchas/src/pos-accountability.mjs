...
export async function branchGate(request,env,personal){
  ...
  const special=p==='/api/pos-actors'||/^\/api\/sales\/[^/]+$/.test(p);
  ...
  if(!allowed)return personal?null:{response:json({error:'Use an individual login to access the dashboard.'},403)};
  ...
  const resource=p.match(/^\/api\/(sales|bookings|daily-closing)\/([^/]+)$/);
  ...
  for(const [id,table] of [[body.customerId,'customers'],[body.bookingId,'bookings']])if(id){const record=await first(env,'SELECT branch_id FROM '+table+' WHERE id=?',[id]);if(!record||record.branch_id!==branchId)return {response:json({error:'Customer or booking belongs to another branch.'},403)};}
...
async function actorAccounts(env,branchId){return (await rows(env,"SELECT u.*,s.name,s.status AS staff_status FROM access_users u LEFT JOIN staff s ON s.id=u.staff_id WHERE u.enabled=1 AND u.role IN ('owner','admin','manager','staff')")).filter(a=>(!a.staff_id||a.staff_status==='Active')&&(a.role==='owner'||a.all_branches||parse(a.branch_ids).includes(branchId)));}
export async function verifyActor(request,env,branchId,elevated=false,reasonRequired=elevated){
  ...
  if(!id||!/^\d{6,12}$/.test(pin))return {response:json({error:'Select your name and enter your individual PIN.'},403)};
  if(!await limit(env,'action-pin:'+await digest(ip+':'+id)))return {response:json({error:'Too many PIN attempts. Try again in 15 minutes.'},429)};
  const account=(await actorAccounts(env,branchId)).find(a=>a.id===id);
  const actual=await hashPin(pin,account?.pin_salt||'dummy-action-pin');
  if(!account||actual!==account.pin_hash||(elevated&&!['owner','admin','manager'].includes(account.role)))return {response:json({error:elevated?'A valid Manager, Admin, or Owner PIN for this branch is required.':'Incorrect staff PIN or no access to this branch.'},403)};
  if(reasonRequired&&!text(body.editReason))return {response:json({error:'Enter a reason for the edit.'},400)};
  await env.DB.prepare('DELETE FROM access_login_limits WHERE key=?').bind('action-pin:'+await digest(ip+':'+id)).run();
...
export async function editSale(request,env,id,paymentLabel){
  ...
  const auth=await verifyActor(request,env,sale.branch_id,true);if(auth.response)return auth.response;
  ...
  if([cash,card].some(n=>!Number.isSafeInteger(n)||n<0)||card>total||cash+card<total)return json({error:'Cash and card must cover the total; card cannot exceed it.'},400);
  const change=cash+card-total,method=paymentLabel(cash,card,total,'');

...
const tabPermissions = { overview:"dashboard",pos:"pos",bookings:"bookings",customers:"customers",services:"services",products:"products",inventory:"inventory",staff:"staff",roster:"roster",reports:"reports",closing:"closing",access:"access",branches:"branches","recent-sales":"pos" };
...
function canViewTab(tab) { return tab === "access" ? canManageAccess() : tab === "reports" ? userCan("reports") || userCan("payroll") : tab === "pos" ? userCan("pos") || userCan("time_clock") : userCan(tabPermissions[tab]); }
function roleName(role) { return ({owner:"Owner",admin:"Admin",manager:"Manager",staff:"Staff",none:"No access"})[role] || "No access"; }
function applyAccessUi() {
  ...
  document.querySelectorAll('#staffForm [name="hourlyRate"],#staffProfileForm [name="hourlyRate"],#staffForm [name^="xero"],#staffProfileForm [name^="xero"]').forEach((input) => { input.disabled = !userCan("payroll",true); input.closest("label").hidden = !userCan("payroll"); });
  document.querySelectorAll(".tab[id]").forEach((section) => {
    const permission = tabPermissions[section.id];
    section.querySelectorAll('form button[type="submit"]').forEach((button) => { if (permission && section.id !== "access") button.disabled = !userCan(permission,true); });
  });
  ...
  document.querySelectorAll('#staffTable tr').forEach((row)=>{ if(row.children[3])row.children[3].hidden=!userCan("payroll"); });
  const payHeader=document.querySelector('#staffTable').closest('table').querySelector('thead tr')?.children[3];if(payHeader)payHeader.hidden=!userCan("payroll");

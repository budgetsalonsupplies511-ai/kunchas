...
async function getManagerData(request, env) {
  ...
  const [branchRows, sales, saleItems, bookings, customers, staff, roster, services, products, inventory, closings] = await Promise.all([
    ...
    permissionSet.has("products") || permissionSet.has("inventory") ? all(env, "SELECT id, name, sku, barcode, category, price_cents, status FROM products WHERE status = 'Active' ORDER BY category, name") : [],
...
<section class="manager-section" id="manager-services"><div class="panel"><div class="section-heading"><div><h2 class="manager-table-title">Services and prices</h2><p class="hint">Search the service catalogue used across all branches.</p></div><label>Search services<input id="managerServiceSearch" type="search" placeholder="Service, category or price"></label></div><div class="table-wrap"><table><thead><tr><th>Category</th><th>Subcategory</th><th>Service</th><th>Duration</th><th>Price</th></tr></thead><tbody id="managerServicesTable"></tbody></table></div></div></section>
...
renderManagerCustomers();setRows("managerStaffTable",managerData.staff,(item)=>'<tr><td><strong>'+managerEsc(item.name)+'</strong></td><td>'+managerEsc(item.role||"Staff")+'</td><td>'+managerEsc(item.phone||"")+'</td><td>'+managerEsc(item.email||"")+'</td><td>'+managerEsc(item.status||"")+'</td></tr>',5);setRows("managerRosterTable",managerData.roster,(item)=>'<tr><td>'+managerEsc(item.roster_date)+'</td><td><strong>'+managerEsc(item.staff_name||"Staff")+'</strong></td><td>'+managerEsc((item.start_time||"")+"–"+(item.end_time||""))+'</td><td>'+managerEsc(item.status||"")+'</td><td>'+managerEsc(item.notes||"")+'</td></tr>',5);renderManagerServices();setRows("managerProductsTable",managerData.products,(item)=>'<tr><td><strong>'+managerEsc(item.name)+'</strong></td><td>'+managerEsc(item.sku||"")+'</td><td>'+managerEsc(item.category||"")+'</td><td>'+managerMoney(item.price_cents)+'</td></tr>',4);renderManagerInventory();document.querySelector("#managerStockProduct").innerHTML='<option value="">Choose product</option>'+(managerData.products||[]).map((item)=>'<option value="'+managerEsc(item.id)+'">'+managerEsc(item.name+(item.sku?" · "+item.sku:""))+'</option>').join("");
...
function renderManagerServices(){const query=document.querySelector("#managerServiceSearch").value.trim().toLowerCase();const rows=(managerData.services||[]).filter((item)=>[item.name,item.category,item.subcategory,managerMoney(item.price_cents)].join(" ").toLowerCase().includes(query));setRows("managerServicesTable",rows,(item)=>'<tr><td>'+managerEsc(item.category||"")+'</td><td>'+managerEsc(item.subcategory||"")+'</td><td><strong>'+managerEsc(item.name)+'</strong></td><td>'+Number(item.duration_minutes||0)+' min</td><td><strong>'+managerMoney(item.price_cents)+'</strong></td></tr>',5);}

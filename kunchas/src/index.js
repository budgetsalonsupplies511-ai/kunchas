...
<body class="${isAdmin ? "admin-mode" : "staff-mode pos-locked"}">
  ...
  <main class="app">
    ...
    <dialog id="changePinDialog" class="branch-dialog branch-action-dialog"><form id="changePinForm"><div class="branch-dialog-header"><h2>Change my PIN</h2></div><div class="branch-dialog-body"><label>Current PIN<input name="currentPin" type="password" inputmode="text" autocomplete="current-password" required></label><label>New PIN<input name="newPin" type="password" inputmode="text" autocomplete="new-password" pattern="[A-Za-z0-9]{4,6}" required></label><label>Confirm new PIN<input name="confirmPin" type="password" inputmode="text" autocomplete="new-password" pattern="[A-Za-z0-9]{4,6}" required></label><p class="hint">Use 4–6 letters or digits. You will sign in again after changing it.</p><p id="changePinMessage" role="alert"></p></div><div class="branch-dialog-footer"><button class="secondary" type="button" id="cancelPinChange">Cancel</button><button class="primary" type="submit">Change PIN</button></div></form></dialog>
    <dialog id="managerDashboardDialog" class="branch-dialog branch-action-dialog"><form id="managerDashboardForm"><div class="branch-dialog-header"><div><p class="eyebrow">Manager access</p><h2>Open manager dashboard</h2></div></div><div class="branch-dialog-body"><label>Manager PIN<input name="pin" type="password" inputmode="text" autocomplete="off" pattern="[A-Za-z0-9]{4,12}" required autofocus></label><p class="hint">Your manager PIN opens only this branch. Menus and editing follow the permissions set by the owner.</p><p id="managerDashboardError" role="alert"></p></div><div class="branch-dialog-footer"><button class="secondary" type="button" id="cancelManagerDashboard">Cancel</button><button class="primary" type="submit">Open dashboard</button></div></form></dialog>
...
@media (min-width:701px) and (max-width:1000px){ body{grid-template-columns:224px minmax(0,1fr)}.sidebar{position:sticky;top:0;align-self:start;width:224px;height:100vh;height:100dvh;min-height:0;overflow-y:auto;overscroll-behavior:contain} }
